 ...
 * ```OUTPUT_LDUST```: possible values are ```0``` or ```1```. The default is ```0```. If set to ```1```, the code will output an additional column in the ```.fout``` file containing the inferred dust luminosity for each galaxy. The dust luminosity is defined as the total energy that was absorbed by dust, and it should be directly comparable to the observed infrared luminosity if the chosen dust law and the assumed dust geometry are accurate. The dust luminosity is written in the file in logarithmic scale, and in units of [total solar luminosity](https://en.wikipedia.org/wiki/Solar_luminosity).
